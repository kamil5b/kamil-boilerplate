...
export function createPaymentDashboardRepository(): PaymentDashboardRepository {
  return {
    async getCustomerPaymentSummaries(client, params) {
      ...
      // Payable: customer owes us (payments from BUY transactions)
      // Receivable: customer paid us (payments from SELL transactions)
      const result = await client.query(
        `SELECT 
          COALESCE(c.id, '00000000-0000-0000-0000-000000000000') as customer_id,
          COALESCE(c.name, 'Anonymous') as customer_name,
          COALESCE(SUM(CASE WHEN t.type = 'BUY' THEN p.amount ELSE 0 END), 0) as payable,
          COALESCE(SUM(CASE WHEN t.type = 'SELL' THEN p.amount ELSE 0 END), 0) as receivable
         FROM payments p
    ...
    async getCustomerPaymentHistorical(client, params) {
      ...
      // Group by date and calculate daily payment totals
      const result = await client.query(
        `SELECT 
          DATE(p.created_at) as date,
          COALESCE(SUM(CASE WHEN t.type = 'BUY' THEN p.amount ELSE 0 END), 0) as payable,
          COALESCE(SUM(CASE WHEN t.type = 'SELL' THEN p.amount ELSE 0 END), 0) as receivable,
          COALESCE(SUM(CASE WHEN t.type = 'SELL' THEN p.amount ELSE 0 END), 0) - 
          COALESCE(SUM(CASE WHEN t.type = 'BUY' THEN p.amount ELSE 0 END), 0) as net
         FROM payments p
         LEFT JOIN transactions t ON p.transaction_id = t.id
         ${whereClause}
